# <img src="https://media.giphy.com/media/hvRJCLFzcasrng815IU/giphy.gif" width="30px"> Hi, I'm Mohammed Hasham

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mohammed-hasham-38765a27a)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:hashim332004@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Hasham-03)
[![Portfolio](https://img.shields.io/badge/Portfolio-FF6B6B?style=for-the-badge&logo=firefox&logoColor=white)](https://d24cp79gc01cvk.cloudfront.net)

</div>

---

<div align="center">

### ☁️ Cloud & DevOps Engineer | 🤖 AI Agent Developer | Freelance LLM Specialist

**Building scalable cloud infrastructure and intelligent AI systems**

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=18&duration=3000&pause=1000&multiline=true&width=700&height=100&lines=☁️+Cloud+Architecture;🔧+DevOps+%26+CI%2FCD;🤖+AI+Agent+Development;🧠+LLM+%26+RAG+Systems;📦+Serverless+Architecture" alt="Typing SVG" />

</div>

---

## 🎯 Who Am I?

<img align="right" src="https://media.giphy.com/media/SWoSkN6DxTszqKSJqv/giphy.gif" alt="Coder" width="280px">

I'm **Mohammed Hasham**, a passionate **Cloud & AI Engineer** currently working as:

🤖 **AI Agent Developer Intern** at **Rooman Technologies**  
💻 **Freelance AI & LLM Specialist**

**What I do:**
- Build intelligent AI agents that solve real-world problems
- Design scalable cloud infrastructure on AWS
- Develop LLM applications with RAG systems
- Create automated DevOps pipelines
- Deploy serverless architectures

**My Focus:**
- **Primary**: Cloud & DevOps (☁️ Architecture, 🔧 Automation, 📦 Serverless)
- **Secondary**: AI & ML (🤖 Agents, 🧠 LLMs, 📚 RAG Systems)

---

## 💻 Skills I Master

### ☁️ Cloud & DevOps
<div>
  <img src="https://img.shields.io/badge/AWS-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/Lambda-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/EC2-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/Fargate-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Terraform-7B42BC?style=for-the-badge&logo=terraform&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub%20Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white" />
</div>

### 🔐 Security & Services
<div>
  <img src="https://img.shields.io/badge/AWS%20Cognito-527FFF?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/IAM-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/API%20Gateway-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/S3-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/CloudFront-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/CloudWatch-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white" />
</div>

### 💾 Databases
<div>
  <img src="https://img.shields.io/badge/DynamoDB-527FFF?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/MongoDB-13AA52?style=for-the-badge&logo=mongodb&logoColor=white" />
</div>

### 🤖 AI & Machine Learning
<div>
  <img src="https://img.shields.io/badge/LLM-FF6B35?style=for-the-badge&logo=openai&logoColor=white" />
  <img src="https://img.shields.io/badge/RAG-4A90E2?style=for-the-badge&logo=databricks&logoColor=white" />
  <img src="https://img.shields.io/badge/AWS%20Bedrock-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/AI%20Agents-00D9FF?style=for-the-badge&logo=robot&logoColor=white" />
  <img src="https://img.shields.io/badge/LangChain-1F7E82?style=for-the-badge&logo=langchain&logoColor=white" />
  <img src="https://img.shields.io/badge/Vector%20DB-6C5CE7?style=for-the-badge&logo=pinecone&logoColor=white" />
</div>

### 🖥️ Programming
<div>
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black" />
</div>

---

## 🚀 My Best Projects

### 1️⃣ Automated Cloud-Native Deployment Pipeline
**Enterprise CI/CD with Infrastructure-as-Code**

I built a complete automated deployment system that:
- 🔄 Runs tests and builds Docker images automatically
- 🚀 Deploys to AWS Fargate without downtime
- 📋 Manages all infrastructure with Terraform
- ⚡ Scales automatically with zero manual intervention

**Tech**: Docker | Terraform | GitHub Actions | AWS ECR | Fargate

[View Code →](https://github.com/Hasham-03/portfolio-fargate)

---

### 2️⃣ GenAI Chat Application - Serverless AI System
**Production-Grade LLM Chat with Secure Authentication**

I created an intelligent chat application that:
- 🧠 Uses Amazon Bedrock for advanced AI responses
- 📚 Implements RAG for context-aware answers
- 🔐 Secures users with AWS Cognito authentication
- 💾 Stores conversations in DynamoDB at scale
- 🔍 Searches through documents with vector embeddings
- ⚡ Responds with low-latency via Lambda

**Architecture**: Bedrock LLM → Lambda → API Gateway → DynamoDB + Cognito → Next.js Frontend

**Tech**: AWS (Lambda, Bedrock, DynamoDB, Cognito, CloudWatch) | Python | LangChain | RAG | Next.js

[Live Demo →](https://main.dtytdzyec2pcz.amplifyapp.com/) | [View Code →](https://github.com/Hasham-03/aws-serverless-ai-app)

---

### 3️⃣ Resume Builder - Full-Stack AWS App
**Dynamic Resume Creation & PDF Generation**

I built a web app where users can:
- ✏️ Create professional resumes in real-time
- 👀 Preview formatted output instantly
- 📥 Download as PDF with one click
- ☁️ Store resumes securely in the cloud

**Deployed on AWS Amplify (frontend) + EC2 (backend with n8n automation)**

**Tech**: Next.js | n8n | AWS Amplify | EC2 | HTTPS

[Live Demo →](https://share.google/kunL76Bvq0HtMIvbR) | [View Code →](https://github.com/Hasham-03/resume-builder)

---

### 4️⃣ Serverless Image Processing
**Automatic Image Resizing on S3 Upload**

I created an event-driven system that:
- 📸 Automatically processes images when uploaded to S3
- 🖼️ Resizes and optimizes images with Python
- 📊 Tracks everything with CloudWatch monitoring
- ⚡ Scales infinitely without managing servers

**Tech**: AWS Lambda | S3 | CloudWatch | Python | Pillow

[View Code →](https://github.com/Hasham-03/aws-serverless-image-resizer)

---

### 5️⃣ Personal Portfolio Website
**Fast, Secure, Global Portfolio Hosting**

I built my portfolio and hosted it on:
- 🌍 AWS CloudFront (global CDN for fast loading)
- 📦 S3 (static file storage)
- 🔐 HTTPS (secure connections)
- 📱 Responsive design (works on all devices)

**Tech**: HTML | CSS | JavaScript | AWS S3 | CloudFront

[Live Demo →](https://d24cp79gc01cvk.cloudfront.net) | [View Code →](https://github.com/Hasham-03/My-Portfolio)

---

## 📊 My GitHub Activity

<div align="center">

![Hasham's GitHub stats](https://github-readme-stats.vercel.app/api?username=Hasham-03&show_icons=true&theme=radical&count_private=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Hasham-03&layout=compact&theme=radical&langs_count=8)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=Hasham-03&theme=radical)

</div>

---

## 💼 My Journey

**🎓 Currently**
- 🤖 **AI Agent Developer Intern** at **Rooman Technologies**
  - Building intelligent AI agents
  - Developing LLM applications
  - Creating RAG systems for enterprise use
  
- 💻 **Freelance AI & LLM Specialist**
  - Custom LLM applications
  - RAG system design
  - AWS cloud consulting
  - AI architecture design

---

## 🤝 Let's Connect

I'm always excited to work on cloud and AI projects!

📧 **Email**: hashim332004@gmail.com  
💼 **LinkedIn**: [Mohammed Hasham](https://www.linkedin.com/in/mohammed-hasham-38765a27a)  
🐙 **GitHub**: [Hasham-03](https://github.com/Hasham-03)  
🌐 **Portfolio**: [d24cp79gc01cvk.cloudfront.net](https://d24cp79gc01cvk.cloudfront.net)

**I'm available for:**
- ☁️ Cloud architecture consulting
- 🔧 AWS DevOps projects
- 🤖 AI Agent development
- 🧠 LLM & RAG implementations
- 💰 Freelance engagements

---

<div align="center">

### ⭐ If you like my work, give it a star!

**Made with ❤️ by Mohammed Hasham**

![Visitor Counter](https://visitor-badge.laobi.icu/badge?page_id=Hasham-03.Hasham-03)

_Updated: March 18, 2026_

</div>
